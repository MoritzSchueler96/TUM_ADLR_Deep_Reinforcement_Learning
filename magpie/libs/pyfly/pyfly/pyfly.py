...
class ConstraintException(Exception):
    def __init__(self, variable, value, limit):
        self.message = "Constraint on {} violated ({}/{})".format(
...
class Variable:
    def __init__(
        ...
        :param convert_to_radians: (bool) whether to convert values for attributes from configuration file from degrees to radians
        :param unit: (string) unit of the state, for plotting purposes [degrees, %]
        ...
        self.np_random = None  # TODO: why not initialize directly?
        self.seed()
    ...
    def reset(self, value=None):
        ...
        if value is None:
            try:
                ...
                raise Exception(
                    "Variable init_min and init_max can not be None if no value is provided on reset"
                )
        else:
    ...
    def set_value(self, value, save=True):
        """
        Set value of state, after applying limits and constraints to value. Raises ConstraintException if constraints are violated
    ...
    def plot(self, axis=None, y_unit=None, target=None, plot_id=None, **plot_kw):
        ...
        :param y_unit: (string) unit state should be plotted in, will convert values if different from internal representation [degrees, %]
        ...
        if "degrees" in y_unit:
            ...
            y_unit == "%"
        ):  # TODO: scale positive according to positive limit and negative according to lowest minimum value
            y = linear_scaling(y, self.value_min, self.value_max, -100, 100)
            if target is not None:
                target["data"] = linear_scaling(
                    target["data"], self.value_min, self.value_max, -100, 100
                )
                if "bound" in target:
                    target["bound"] = linear_scaling(
                        target["bound"], self.value_min, self.value_max, -100, 100
                    )
        else:
            y = y
        ...
        # plot data if plot doesn't exist already otherwise change data tobe plotted
        if self.lines.get(plot_id, None) is None:
            (line,) = plot_object.plot(x, y, label=self.label, **plot_kw)
            self.lines[plot_id] = line

            if target is not None:
                (tar_line,) = plot_object.plot(
                    x,
                    target["data"],
                    color=self.lines[plot_id].get_color(),
                    linestyle="dashed",
                    marker="x",
                    markevery=0.2,
                )
                ...
                if "bound" in target:
                    tar_bound = plot_object.fill_between(
                        np.arange(target["bound"].shape[0]),
            ...
            if target is not None:
                ...
                if "bound" in target:  # TODO: fix this?
                    self.target_bounds[plot_id].set_data(
                        np.arange(target["bound"].shape[0]),
                        target["data"] + target["bound"],
                        target["data"] - target["bound"],
                    )
...
class ControlVariable(Variable):
    def __init__(
        ...
        **kwargs
        ...
        if order == 1:
            ...
            self.coefs = [
                [0, 1, 0],
                [-self.omega_0 ** 2, -2 * self.zeta * self.omega_0, self.omega_0 ** 2],
            ]
        self.dot = None
        self.command = None
    ...
    def apply_conditions(self, values):  # TODO: rename to apply_constraints?
        """
        Apply state limits and constraints to value. Will raise ConstraintException if constraints are violated
    ...
    def set_command(self, command):  # TODO: rename to values?
        """
    ...
    def set_value(self, value, save=True):  # TODO: rename to values?
        """
        Set value of state, after applying limits and constraints to value. Raises ConstraintException if constraints are violated
    ...
    def get_coeffs(self):  # TODO: really needed? If yes, describe here why it is
        if self.order == 1:
            return
        else:
            return []
...
class EnergyVariable(Variable):
    ...
    def calculate_value(self):
        ...
        if (
            self.name == "energy_kinetic_rotational"
            or self.name == "energy_kinetic"
            or self.name == "energy_total"
        ):
            for i, axis in enumerate(["omega_p", "omega_q", "omega_r"]):
                m_i = self.inertia_matrix[i, i]
                val += 1 / 2 * m_i * self.variables[axis].value ** 2
        if (
            self.name == "energy_kinetic_translational"
            or self.name == "energy_kinetic"
            or self.name == "energy_total"
        ):
...
class Actuation:
    def __init__(self, model_inputs, actuator_inputs, dynamics):
        """
        PyFly actuation object, responsible for verifying validity of configured actuator model, processing inputs and actuator dynamics.
    ...
    def set_states(self, values, save=True):
        ...
        if self.elevon_dynamics:
            elevator, aileron = self._map_elevon_to_elevail(
                er=self.states["elevon_right"].value,
    ...
    def add_state(self, state):
        ...
        if state.name in self.dynamics:
            for i in range(2):
                for j in range(3):
                    self.coefficients[i][j] = np.append(
                        self.coefficients[i][j], state.coefs[i][j]
                    )
    ...
    def rhs(self, setpoints=None):
        ...
        # TODO: describe
        dot = (
            np.multiply(states, self.coefficients[0][0])
            + np.multiply(setpoints, self.coefficients[0][2])
            + np.multiply(dots, self.coefficients[0][1])
        )
        ddot = (
            np.multiply(states, self.coefficients[1][0])
            + np.multiply(setpoints, self.coefficients[1][2])
            + np.multiply(dots, self.coefficients[1][1])
        )
    ...
    def set_and_constrain_commands(self, commands):
        """
        Take  raw actuator commands and constrain them according to the state limits and constraints, and update state values and history.
        ...
        if self.elevon_dynamics and "elevator" and "aileron" in self.inputs:
            elev_c, ail_c = (
                commands[self.input_indices["elevator"]],
                commands[self.input_indices["aileron"]],
            )
        ...
        if self.elevon_dynamics:
            elev_c, ail_c = self._map_elevon_to_elevail(
                er=dynamics_commands["elevon_right"],
    ...
    def finalize(self):
        ...
        if "elevon_left" in self.dynamics or "elevon_right" in self.dynamics:
            assert (
                "elevon_left" in self.dynamics
                and "elevon_right" in self.dynamics
                and not ("aileron" in self.dynamics or "elevator" in self.dynamics)
            )
            assert "elevon_left" in self.states and "elevon_right" in self.states
            self.elevon_dynamics = True

            # Set elevator limits from elevon limits for plotting purposes etc.
            if "elevator" in self.states:
                elev_min, _ = self._map_elevon_to_elevail(
                    er=self.states["elevon_right"].value_min,
                    el=self.states["elevon_left"].value_min,
                )
                elev_max, _ = self._map_elevon_to_elevail(
                    er=self.states["elevon_right"].value_max,
                    el=self.states["elevon_left"].value_max,
                )
                self.states["elevator"].value_min = elev_min
                self.states["elevator"].value_max = elev_max

            # Set aileron limits from elevon limits for plotting purposes etc.
            if "aileron" in self.states:
                _, ail_min = self._map_elevon_to_elevail(
                    er=self.states["elevon_right"].value_max,
                    el=self.states["elevon_left"].value_min,
                )
                _, ail_max = self._map_elevon_to_elevail(
                    er=self.states["elevon_right"].value_min,
                    el=self.states["elevon_left"].value_max,
                )
    ...
    def reset(self, state_init=None):
        ...
        if self.elevon_dynamics:
            elev, ail = self._map_elevon_to_elevail(
                er=self.states["elevon_right"].value,
                el=self.states["elevon_left"].value,
            )
...
class AttitudeQuaternion:
    ...
    def _from_euler_angles(self, euler):
        ...
        # calculate attitude quaternion representation
        e0 = np.cos(psi / 2) * np.cos(theta / 2) * np.cos(phi / 2) + np.sin(
            psi / 2
        ) * np.sin(theta / 2) * np.sin(phi / 2)
        e1 = np.cos(psi / 2) * np.cos(theta / 2) * np.sin(phi / 2) - np.sin(
            psi / 2
        ) * np.sin(theta / 2) * np.cos(phi / 2)
        e2 = np.cos(psi / 2) * np.sin(theta / 2) * np.cos(phi / 2) + np.sin(
            psi / 2
        ) * np.cos(theta / 2) * np.sin(phi / 2)
        e3 = np.sin(psi / 2) * np.cos(theta / 2) * np.cos(phi / 2) - np.cos(
            psi / 2
        ) * np.sin(theta / 2) * np.sin(phi / 2)
...
class Wind:
    def __init__(
        ...
        # use DrydenGustModel if turbulence is present
        if self.turbulence:
            self.dryden = DrydenGustModel(
                self.turbulence_sim_length, dt, b, intensity=turbulence_intensity
            )
        else:
            self.dryden = None

        self.np_random = None  # TODO: needed?
        self.seed()
    ...
    def reset(self, value=None, noise=None):  # TODO: rename to magnitude?
        """
...
class Plot:
    ...
    def plot(self, fig=None, targets=None):
        ...
        if self.axis is None:
            ...
            if fig is not None:
                self.axis = {
                    self.y_units[0]: plt.subplot(
                        fig, title=self.title, xlabel=xlabel, ylabel=ylabel
                    )
                }
            else:
                self.axis = {
                    self.y_units[0]: plt.plot(
        ...
        for i, v in enumerate(self.variables):
            ...
            target = (
                targets[v.name] if targets is not None and v.name in targets else None
            )
            ...
            if len(self.y_units) > 1:
                ...
                for ax in self.axis.values():
                    labeled_lines.extend(
                        [l for l in ax.lines if "_line" not in l.get_label()]
                    )
                self.axis[self.y_units[0]].legend(
                    labeled_lines, [l.get_label() for l in labeled_lines]
                )
            else:
                self.axis[self.y_units[0]].legend()
        ...
        # set ticks according to unit
        if self.x_unit == "seconds":
            xticks = self.axis[self.y_units[0]].get_xticks()
            self.axis[self.y_units[0]].set_xticklabels(
                ["{0:.1f}".format(tick * self.dt) for tick in xticks]
            )
...
class PyFly:
    ...
    def __init__(
        ...
        """
        A flight simulator for fixed wing aircraft with configurable initial conditions, constraints and turbulence conditions
        ...
        # get filetype of parameter file and load
        ...
        # calculate gammas #TODO: describe
        self.gammas = [self.I[0, 0] * self.I[2, 2] - self.I[0, 2] ** 2]
        self.gammas.append(
            (np.abs(self.I[0, 2]) * (self.I[0, 0] - self.I[1, 1] + self.I[2, 2]))
            / self.gammas[0]
        )
        self.gammas.append(
            (self.I[2, 2] * (self.I[2, 2] - self.I[1, 1]) + self.I[0, 2] ** 2)
            / self.gammas[0]
        )
        ...
        self.gammas.append(
            ((self.I[0, 0] - self.I[1, 1]) * self.I[0, 0] + self.I[0, 2] ** 2)
            / self.gammas[0]
        )
        ...
        # open config file and set parameters
        ...
        self.model_inputs = [
            ...
            "rudder",
            "throttle",
        ]  # TODO: rudder is never used?
        self.energy_states = []

        # get required variables and check if some are missing -> Exception
        if not set(self.REQUIRED_VARIABLES).issubset(
            [v["name"] for v in self.cfg["variables"]]
        ):
            raise Exception(
                "Missing required variable(s) in config file: {}".format(
                    ",".join(
                        list(
                            set(self.REQUIRED_VARIABLES)
                            - set([v["name"] for v in self.cfg["variables"]])
                        )
                    )
                )
            )
        ...
        self.g = self.cfg["g"]
        self.wind = Wind(
            mag_min=self.cfg["wind_magnitude_min"],
            mag_max=self.cfg["wind_magnitude_max"],
            turbulence=self.cfg["turbulence"],
            turbulence_intensity=self.cfg["turbulence_intensity"],
            sim_length=self.cfg.get("turbulence_sim_length", 300),
            dt=self.cfg["dt"],
            b=self.params["b"],
        )

        # TODO: move up to state init?
        self.state["attitude"] = AttitudeQuaternion()
        self.attitude_states_with_constraints = []

        # set UAV model type? # TODO: describe better/correct
        ...
        for v in self.cfg["variables"]:
            if v["name"] in self.attitude_states and any(
                [
                    v.get(attribute, None) is not None
                    for attribute in [
                        "constraint_min",
                        "constraint_max",
                        "value_min",
            ...
            if v["name"] in self.actuator_states:
                ...
                self.state[v["name"]] = EnergyVariable(
                    mass=self.params["mass"], inertia_matrix=self.I, gravity=self.g, **v
                )
            else:
        ...
        # check if config is valid and set actuator state limits
        ...
        for energy_state in self.energy_states:
            for req_var_name in self.state[energy_state].required_variables:
                self.state[energy_state].add_requirement(
                    req_var_name, self.state[req_var_name]
                )

        # TODO: check that all plotted variables are declared in cfg.variables
        # get plot config -> which plots to plot
        self.plots = []
    ...
    def reset(self, state=None, turbulence_noise=None):  # TODO: recursive function?
        ...
        for name, var in self.state.items():
            if (
                name in ["Va", "alpha", "beta", "attitude"]
                or "wind" in name
                or "energy" in name
                or isinstance(var, ControlVariable)
            ):
                continue
        ...
        # reset wind to last state if exists
        wind_init = None
        ...
        vel = np.array(
            self.get_states_vector(["velocity_u", "velocity_v", "velocity_w"])
        )

        # reset states to last state
    ...
    def render(self, mode="plot", close=False, viewer=None, targets=None, block=False):
        # TODO: add spacing between plots
        """
        ...
        if mode == "plot":
            ...
            for i, p in enumerate(self.plots):
                ...
                if targets is not None:
                    ...
                    p.plot(
                        fig=sub_fig,
                        targets={k: v for k, v in targets.items() if k in plot_targets},
                    )
                else:
                    p.plot(fig=sub_fig)
    ...
    def step(self, commands):
        ...
        try:
            sol = scipy.integrate.solve_ivp(
                fun=lambda t, y: self._dynamics(t, y), t_span=(0, self.dt), y0=y0
            )
            ...
            for energy_state in self.energy_states:
                self.state[energy_state].set_value(
                    self.state[energy_state].calculate_value(), save=True
                )
    ...
    def _forces(self, attitude, omega, vel, controls):
        ...
        C_L_alpha = (1 - sigma) * C_L_alpha_lin + sigma * (
            2 * np.sign(alpha) * np.sin(alpha) ** 2 * np.cos(alpha)
        )

        f_lift_s = pre_fac * (
            C_L_alpha
            + self.params["C_L_q"] * self.params["c"] / (2 * Va) * q
            + self.params["C_L_delta_e"] * elevator
        )
        # C_D_alpha = self.params["C_D_0"] + self.params["C_D_alpha1"] * alpha + self.params["C_D_alpha2"] * alpha ** 2
        C_D_alpha = (
            C_D_p
            + (1 - sigma)
            * (self.params["C_L_0"] + self.params["C_L_alpha"] * alpha) ** 2
            / (np.pi * e * ar)
            + sigma * (2 * np.sign(alpha) * math.pow(np.sin(alpha), 3))
        )
        C_D_beta = (
            self.params["C_D_beta1"] * beta + self.params["C_D_beta2"] * beta ** 2
        )
        f_drag_s = pre_fac * (
            C_D_alpha
            + C_D_beta
            + self.params["C_D_q"] * self.params["c"] / (2 * Va) * q
            + self.params["C_D_delta_e"] * elevator ** 2
        )

        C_m = (1 - sigma) * (C_m_0 + C_m_alpha * alpha) + sigma * (
            C_m_fp * np.sign(alpha) * np.sin(alpha) ** 2
        )
        m = (
            pre_fac
            * self.params["c"]
            * (
                C_m
                + self.params["C_m_q"] * self.params["b"] / (2 * Va) * q
                + self.params["C_m_delta_e"] * elevator
            )
        )

        f_y = pre_fac * (
            self.params["C_Y_0"]
            + self.params["C_Y_beta"] * beta
            + self.params["C_Y_p"] * self.params["b"] / (2 * Va) * p
            + self.params["C_Y_r"] * self.params["b"] / (2 * Va) * r
            + self.params["C_Y_delta_a"] * aileron
            + self.params["C_Y_delta_r"] * rudder
        )
        l = (
            pre_fac
            * self.params["b"]
            * (
                self.params["C_l_0"]
                + self.params["C_l_beta"] * beta
                + self.params["C_l_p"] * self.params["b"] / (2 * Va) * p
                + self.params["C_l_r"] * self.params["b"] / (2 * Va) * r
                + self.params["C_l_delta_a"] * aileron
                + self.params["C_l_delta_r"] * rudder
            )
        )
        n = (
            pre_fac
            ...
            * (
                self.params["C_n_0"]
                + self.params["C_n_beta"] * beta
                + self.params["C_n_p"] * self.params["b"] / (2 * Va) * p
                + self.params["C_n_r"] * self.params["b"] / (2 * Va) * r
                + self.params["C_n_delta_a"] * aileron
                + self.params["C_n_delta_r"] * rudder
            )
        )

        f_aero = np.dot(
            self._rot_b_v(np.array([0, alpha, beta])),
            np.array([-f_drag_s, f_y, -f_lift_s]),
        )
        ...
        tau_prop = np.array(
            [-self.params["k_T_P"] * (self.params["k_Omega"] * throttle) ** 2, 0, 0]
        )
    ...
    def _f_omega_dot(self, t, omega, tau):
        ...
        return np.array(
            [
                self.gammas[1] * omega[0] * omega[1]
                - self.gammas[2] * omega[1] * omega[2]
                + self.gammas[3] * tau[0]
                + self.gammas[4] * tau[2],
                self.gammas[5] * omega[0] * omega[2]
                - self.gammas[6] * (omega[0] ** 2 - omega[2] ** 2)
                + tau[1] / self.I[1, 1],
                self.gammas[7] * omega[0] * omega[1]
                - self.gammas[1] * omega[1] * omega[2]
                + self.gammas[4] * tau[0]
                + self.gammas[8] * tau[2],
            ]
        )
        ...
        if len(attitude) == 3:
            ...
            return np.array(
                [
                    ...
                    [
                        np.sin(phi) * np.sin(th) * np.cos(psi)
                        - np.cos(phi) * np.sin(psi),
                        np.sin(phi) * np.sin(th) * np.sin(psi)
                        + np.cos(phi) * np.cos(psi),
                        np.sin(phi) * np.cos(th),
                    ],
                    [
                        np.cos(phi) * np.sin(th) * np.cos(psi)
                        + np.sin(phi) * np.sin(psi),
                        np.cos(phi) * np.sin(th) * np.sin(psi)
        ...
        return np.array(
            [
                ...
                [
                    ...
                    np.sin(phi) * np.sin(th) * np.sin(psi) + np.cos(phi) * np.cos(psi),
    ...
    def _set_states_from_ode_solution(self, ode_sol, save):
        ...
        if save:
            ...
            for state in self.attitude_states_with_constraints:
                self.state[state].set_value(
                    self.state["attitude"].as_euler_angle(state)
                )
        start_i = 4
...
if __name__ == "__main__":
    # import wind model and pid controller
    ...
    for i in range(500):
        ...
        omega = [
            pfly.state["omega_p"].value,
            pfly.state["omega_q"].value,
            pfly.state["omega_r"].value,
        ]
        ...
        # cancel if sim step wasn't successful
        if not success:
            break
